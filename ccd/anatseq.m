...
%         end
    ...
    case 'open file'        % save images to TIF file
        ...
        if (exist(file_par.file_path, 'dir'))
            ...
            [file_name, file_path] = uiputfile(file_name, 'Save Images to File');
            warning('MATLAB:anatseq', '%s not found, saving file to %s',  file_par.file_path, old_dir)
        end
    ...
    otherwise               % invalid command
        warning('MATLAB:anatseq', 'command ''%s'' not recognized', control_command);
end
return
...
if (istype(h_axes, 'axes'))
    ...
    warning('MATLAB:anatseq', 'unable to find image axes');
    return
end
h_text = fig_handle.h_text;
if (~istype(h_text, 'uicontrol'))
    warning('MATLAB:anatseq', 'unable to find text label');
    return
end
...
if (istype(h_axes, 'axes'))
    ...
    warning('MATLAB:anatseq', 'unable to find colorbar axes');
    return
end
...
if (istype(h_image, 'image'))
    ...
    set(h_image, 'EraseMode', 'none', ...
        'ButtonDownFcn', 'anatseq(gcf, ''colormap'')');
...
if (isempty(file_par.file_name))
    set(h_button(7), 'Enable', 'on');
    set(h_button(8), 'Enable', 'off');
else
    set(h_button(7), 'Enable', 'off');
    set(h_button(8), 'Enable', 'on');
end
return
...
button_pos = [0.08 0.03 0.72 0.05]; % button panel position (% of figure)
...
h_fig = figure('Color', figure_bkgnd, ...
    ...
    'Position', figure_pos, ...
    'BackingStore', 'off', ...
    'CloseRequestFcn', 'anatseq(gcf, ''finish'')', ...
    'MenuBar', 'none', ...
    'Name', 'No file open', ...
    'NumberTitle', 'off', ...
    'Pointer', 'arrow', ...
...
% create button panel
labels = {'Snap Image', 'Start Focus', 'Stop Focus', 'Parameters', 'Camera Setup', 'ROI Setup', 'Open File', 'Close File'};
h_button = guipanel(h_fig, button_pos, 'horizontal', ...
    'Enable', 'off', ...
    'FontName', 'Helvetica', ...
    'FontSize', 8, ...
    'Style', 'pushbutton', ...
    'Callback', 'anatseq(gcf, get(gcbo, ''Tag''))', ...
    'String', labels, ...
    'Tag', labels);
set(h_button(3), 'Callback', 'set(gcf, ''Tag'', ''stop'')');
