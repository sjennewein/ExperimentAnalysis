...
MeasurementFolder = 'Exp2\';
RunName = '200Burst_Macro_S01_S613'; %S = 0.1 for the timeharp and S=6.13 for the picture
...
%Initialize Timeharp
InitializeStandard(1000,20,20,-100,0,2);
...
while(1==1)
    ...
    for i = 1:cyclesPerRun
        summedPicture = summedPicture + cast(roiparse(pictures{i},roi_struct),'int32') - cast(bckg, 'int32');
    ...
    save(fileLocation,'summedPicture','summedHistogram','resolution', 'roi_struct', 'cyclesPerRun', 'pictures');
    disp('Data saved');
    run = run + 1;
end
pvcamclose(h_cam);