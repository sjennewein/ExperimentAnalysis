function [Ix, Iy]=Time_Freq_study(Ncloud,Ndip,DeltaSpan)
Gam=2*pi*6.065*10^(6);
Tau=1/Gam;
...
I1x=zeros(1,length(tspan));
Ix=zeros(length(DeltaSpan),length(tspan));
Iy=zeros(length(DeltaSpan),length(tspan));
