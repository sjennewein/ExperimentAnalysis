...
mxArray *pvcam_acquire_timeout(int16 hcam, uns16 nimage, uns16 nregion, rgn_type *region, uns32 exptime, int16 expmode);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
	...
	if (pl_cam_check(hcam)) {
		plhs[0] = pvcam_acquire_timeout(hcam, nimage, nregion, region, exptime, expmode);
	}
	else {
...
mxArray *pvcam_acquire_timeout(int16 hcam, uns16 nimage, uns16 nregion, rgn_type *region, uns32 exptime, int16 expmode) {
	...
	//////////////////////////////////////////////////////////
	// Added for Aspherix to treat timeouts of the camera  ///
	//////////////////////////////////////////////////////////
	SYSTEMTIME 		t,t2;
	FILETIME 		t_temp,t2_temp;
	ULARGE_INTEGER  t_ui,t2_ui;
	__int64			t_start, t_stop;
	...
	status = -1;
	GetSystemTime(&t);
	SystemTimeToFileTime(&t,&t_temp);
	t_ui.LowPart = t_temp.dwLowDateTime;
	t_ui.HighPart = t_temp.dwHighDateTime;
	t_start = t_ui.QuadPart;
	while ((status != READOUT_COMPLETE) && (status != READOUT_NOT_ACTIVE) && (status != READOUT_FAILED)) {
        ...
        Sleep(1);
		GetSystemTime(&t2);
		SystemTimeToFileTime(&t2,&t2_temp);
		t2_ui.LowPart = t2_temp.dwLowDateTime;
		t2_ui.HighPart = t2_temp.dwHighDateTime;
		t_stop = t2_ui.QuadPart;
		t_stop -= t_start;
		if((t_stop /= 10000) >= 900){   //after how many ms should it timeout
			pvcam_error(hcam, "Cannot check camera status during exposure");
			mxDestroyArray(data_struct);
			return(empty_struct);			
		}
